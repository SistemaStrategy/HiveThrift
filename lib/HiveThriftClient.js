(function () {
	...
	HiveThriftClient.prototype.getSchemasNames = function getSchemasNames (session, callback) {
		...
		getSchemasThrift(session, function (error, response) {
			if (error) {
				...
				logger.trace('getSchemas done');
				HiveThriftClient.prototype.fetchRows(response.operationHandle, 50, function (error, response) {
					var schemaNamesTab = new String(response.results.columns[0].stringVal.values).split(',');
					callback(error, schemaNamesTab);
				})
			}
		});
	}
	...
	*/
	HiveThriftClient.prototype.getTablesNames = function getTablesNames (session, schemaName, callback) {
		logger.trace('getTables : ' + JSON.stringify(session));
	
		getTablesThrift(session, schemaName, function (error, response) {
			if (error) {
				logger.trace("getTables error = " + JSON.stringify(error));
				callback(error, response);
			} else {
				logger.trace('getTables done');
				HiveThriftClient.prototype.fetchRows(response.operationHandle, 50, function (error, response) {
					var tableNamesTab = new String(response.results.columns[2].stringVal.values).split(',');
					callback(error, tableNamesTab);
				})
			}
		});
	}
